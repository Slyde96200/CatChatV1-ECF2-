 ...
 */
    $db->query("INSERT INTO santestatus (content, user_id) VALUES ('$content', '$user_id')");
    if ($db->error) {
    ...
    header("Location:santeindex.php");
}
?>

<!-- <?php if (Session::isConnected()) : ?>
 -->
    <form action="santeindex.php?controller=status&task=upload" method="POST" id="write-status" enctype="multipart/form-data" name="mon-formulaire">
        <div class="form-row">
            <textarea name="content" id="content" required placeholder="Une idée de topic ?"></textarea>
        </div>
<!--         <div class="form-row">
...
<section id="status">
    ...
    <?php foreach ($santestatus as $santestat) : ?>
        <h2><?= $santestat['content'] ?></h2><br>
        <article>
            <figure>

                <div class="metadata">
                    <small>Le <?= $santestat['createdAt'] ?></small> |
                    <a href="santeindex.php?controller=status&task=show&id=<?= $santestat['id'] ?>">
                        <i class="ri-chat-3-line"></i> <strong><?= $santestat['comments'] ?></strong> réponses
                    </a>
                </div>
            </div>
        </article>
