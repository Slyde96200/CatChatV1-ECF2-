 ...
 */
    $db->query("INSERT INTO sportstatus (content, user_id) VALUES ('$content', '$user_id')");
    if ($db->error) {
    ...
    header("Location:sportindex.php");
}
?>

<?php if (Session::isConnected()) : ?>

    <form action="sportindex.php?controller=status&task=upload" method="POST" id="write-status" enctype="multipart/form-data" name="mon-formulaire">
        <div class="form-row">
...
<section id="status">
    ...
    <?php foreach ($sportstatus as $sportstat) : ?>
        <h2><?= $sportstat['content'] ?></h2><br>
        <article>
            <figure>

                <div class="metadata">
                    <small>Le <?= $sportstat['createdAt'] ?></small> |
                    <a href="sportindex.php?controller=status&task=show&id=<?= $sportstat['id'] ?>">
                        <i class="ri-chat-3-line"></i> <strong><?= $sportstat['comments'] ?></strong> réponses
                    </a>
                </div>
            </div>
        </article>
