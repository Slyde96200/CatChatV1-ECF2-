 ...
 */
    $db->query("INSERT INTO activitestatus (content, user_id) VALUES ('$content', '$user_id')");
    if ($db->error) {
    ...
    header("Location:activiteindex.php");
}
?>

<?php if (Session::isConnected()) : ?>

    <form action="activiteindex.php?controller=status&task=upload" method="POST" id="write-status" enctype="multipart/form-data" name="mon-formulaire">
        <div class="form-row">
            <textarea name="content" id="content" required placeholder="Un contenu à partager ?"></textarea>
        </div>
        <div class="form-row">
...
<section id="status">
    ...
    <?php foreach ($activitestatus as $activitestat) : ?>
        <h2><?= $activitestat['content'] ?></h2><br>
        <article>
            <figure>

                <div class="metadata">
                    <small>Le <?= $activitestat['createdAt'] ?></small> |
                    <a href="activiteindex.php?controller=status&task=show&id=<?= $activitestat['id'] ?>">
                        <i class="ri-chat-3-line"></i> <strong><?= $activitestat['comments'] ?></strong> réponses
                    </a>
                </div>
            </div>
        </article>
